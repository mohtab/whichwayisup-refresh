...
class Level:
  ...
  def __init__(self, screen, level_name = "w0-l0"):
    ...
    with open(data.levelpath(self.level_name), encoding="utf8") as source:
      conffile = source.readlines()
  ...
  def rebuild_spatial(self):
    self.spatial = {}
    self.tile_order = {id(t): i for i, t in enumerate(self.active_tiles)}
    self.cached_ground_check.clear()
    for t in self.active_tiles:
      t.rect.center = (int(t.x), int(t.y))
      for x in range(t.rect.left // TILE_DIM, (t.rect.right - 1) // TILE_DIM + 1):
        for y in range(t.rect.top // TILE_DIM, (t.rect.bottom - 1) // TILE_DIM + 1):
          self.spatial.setdefault((x,y), []).append(t)

  def candidates(self, rect):
    found = {}
    for x in range(rect.left // TILE_DIM, rect.right // TILE_DIM + 1):
      for y in range(rect.top // TILE_DIM, rect.bottom // TILE_DIM + 1):
        for tile in self.spatial.get((x,y), ()):
          found[id(tile)] = tile
    return sorted(found.values(), key=lambda t: self.tile_order[id(t)])
  ...
  def ground_check(self, x, y):
    if x > SCREEN_WIDTH or y > SCREEN_HEIGHT or x < 0 or y < 0:
      return True
    return any(t.rect.collidepoint(x, y) for t in self.spatial.get((int(x)//TILE_DIM,int(y)//TILE_DIM), ()))
  ...
  def collide(self, rect, dy, dx, topcollision = True):
    ...
    region = rect.inflate(int(abs(dx)*2)+6, int(abs(dy)*2)+6)
    for t in self.candidates(region):
      if not t.is_aligned():
