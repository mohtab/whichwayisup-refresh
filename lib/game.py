...
def steps(screen, level_name="w0-l0", score_mod=0, score=None, joystick=None, driver=None):
  ...
  while (end_trigger == END_NONE or fading):
    ...
    for event in (driver.events if driver is not None else pygame.event.get()):
      if event.type == QUIT:
    ...
    inputs = driver.inputs if driver is not None else parse_inputs(joystick)
    ...
    if player.current_animation == "walking" and not paused and len(particles) < 256:
    ...
    yield {"level": level, "objects": objects, "particles": particles,
           "player": player, "score": score, "dialogue": text if scripted_event_on else None,
           "phase": phase, "fading": fading, "fade": Util.fade_state,
           "scripted": scripted_event_on, "changing": changing_level}
...
def run(screen, level_name="w0-l0", score_mod=0, score=None, joystick=None):
  """Compatibility entry point; the refresh app drives steps independently."""
  clock = pygame.time.Clock()
  simulation = steps(screen, level_name, score_mod, score, joystick)
  while True:
    try:
      next(simulation)
